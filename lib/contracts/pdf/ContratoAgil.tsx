...
export interface ContratoAgilProps {
  ...
  cliente: { nombre: string; monto: number }
  aval: { nombre: string }
  montoTotal: number
  fechaFirma: Date
  ...
  plazoDias?: number   // 24 por default
}

/**
 * Contrato + pagaré para Crédito Ágil (24 días hábiles).
 * Texto literal de CONTRATO_AGIL.docx — incluyendo los typos del
 * original. El producto se llama internamente "CRÉDITO INDIVIDUAL"
 * en la plantilla porque desde el ángulo legal el Ágil ES un
 * individual con aval; la diferencia es operativa (frecuencia diaria),
 * no contractual.
 */
export function ContratoAgil(props: ContratoAgilProps) {
  const {
    numeroContrato, cliente, aval, montoTotal,
  ...
  const CLIENTE = cliente.nombre.toUpperCase()
  const AVAL = aval.nombre.toUpperCase()
  const MONTO = formatCurrency(montoTotal)
  const MONTO_CRED = formatCurrency(cliente.monto)
  ...
  return (
    ...
    >
      ...
      <View style={{ position: 'relative', minHeight: 80, marginBottom: 8 }}>
        ...
        <Text style={styles.title}>PAGARE INDIVIDUAL CON AVAL</Text>
      </View>
      ...
      <Text style={styles.upper}>
        ...
        CANTIDAD QUE SE LE FUE OTORGADO A: {CLIENTE} DÍA {fechaDesembolso} EN LA CIUDAD DE{' '}
        {CIUDAD}, MÉXICO LA CANTIDAD {MONTO} ({montoLetras}) (CAPITAL FINAL A PAGAR).
      </Text>

      <Text style={styles.upper}>
        VALOR RECIBIDO A NUESTRA ENTERA SATISFACCIÓN. ESTE PAGARÉ FORMA PARTE A UNA SERIE
        ENUMERADA DE 1 AL 24 Y TODOS ESTÁN SUJETOS A LA CONDICIÓN DE QUE AL NO PAGARSE
        CUALQUIERA DE ELLOS EN FORMA DIARIA SU VENCIMIENTO SERÁN EXIGIBLES TODOS LOS QUE SIGAN
        EN NÚMERO, ADEMÁS DE LOS YA VENCIDOS DESDE LA FECHA DE VENCIMIENTO DE ESTE DOCUMENTO
        HASTA EL DÍA DE SU LIQUIDACIÓN Y CAUSARÁN INTERESES MORATORIOS AL TIPO DE{' '}
        {interesMoratorio.toFixed(0)}% MENSUAL PAGADERO EN ESTA CIUDAD JUNTAMENTE CON EL
        PRINCIPAL.
      </Text>
      ...
      <View style={styles.table}>
        <View style={styles.tableHeader}>
          <Text style={[styles.tableCellHeader, { flex: 4 }]}>CLIENTE</Text>
        ...
        <View style={styles.tableRow}>
          <Text style={[styles.tableCell, { flex: 4 }]}>{CLIENTE}</Text>
          <Text style={[styles.tableCell, { flex: 1.5 }]}>{MONTO_CRED}</Text>
          <Text style={[styles.tableCell, { flex: 2 }]}> </Text>
          <Text style={[styles.tableCell, { flex: 1.5 }]}> </Text>
        </View>
        <View style={styles.tableRowLast}>
          <Text style={[styles.tableCell, { flex: 4 }]}>AVAL: {AVAL}</Text>
          <Text style={[styles.tableCell, { flex: 1.5 }]}> </Text>
          <Text style={[styles.tableCell, { flex: 2 }]}> </Text>
          <Text style={[styles.tableCell, { flex: 1.5 }]}> </Text>
        </View>
      ...
      <Text style={styles.upper}>
        CONTRATO DE {RL} PARA EL PRODUCTO DENOMINADO &quot;CRÉDITO INDIVIDUAL&quot; QUE CELEBRA
      ...
      <Text style={styles.upper}>
        MANIFIESTAN LIBREMENTE SU VOLUNTAD PARA CELEBRAR EL PRESENTE CONTRATO EN LOS TÉRMINOS
        DE LAS SIGUIENTES:
      </Text>
      ...
      <Text style={styles.upper}>
        ...
        DE {MONTO} ({montoLetras}). EL CLIENTE &quot;ESTÁ DE ACUERDO CON EL MONTO SEÑALADO Y SE
        OBLIGAN A CUBRIR EN LA FORMA, TÉRMINOS Y CONDICIONES CONVENIDAS EN ESTE CONTRATO,
        OBLIGÁNDOSE A RESTITUIR A &quot;{RL}&quot;. EL IMPORTE DEL CRÉDITO MÁS IMPUESTOS,
        INTERESES Y GASTOS QUE SE ESTIPULEN O GENEREN HASTA EL DÍA DE LIQUIDACIÓN TOTAL DEL
        CRÉDITO.
      </Text>
      ...
      <Text style={styles.upper}>
        ...
        EL PRESENTE CONTRATO TENDRÁ COMO PLAZO 24 DIAS HABILES, DENTRO DEL CUAL &quot;EL
        CLIENTE&quot; DEBERÁN LIQUIDAR A &quot;{RL}&quot; EL CRÉDITO O ANTES DEL PLAZO SI
        INCURREN EN CUALQUIERA DE LAS CAUSALES DE VENCIMIENTO ANTICIPADO ESTABLECIDAS; NO
        OBSTANTE, SU TERMINACIÓN, ESTE CONTRATO PRODUCIRÁ TODOS SUS EFECTOS LEGALES, HASTA QUE
        LOS CLIENTES HAYAN LIQUIDADO EN SU TOTALIDAD TODAS LAS CANTIDADES ADECUADAS QUE EXISTAN
        A SU CARGO.
      </Text>
      ...
      <Text style={[styles.upper, { marginTop: 12 }]}>
        ...
        FIRMAN EN LA CIUDAD DE {CIUDAD}, MEXICO, A {fechaDesembolso} QUEDANDO EN EJEMPLAR DE
